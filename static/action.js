...
async function fetchData(){
    ...
    let locations = await fetch(`https://s3.codeths.dev/bell/locations/${instance}`, {cache: 'no-cache'})
    return {day: await day.json(), now: await now.json(), locations: await locations.json(), schedule: schedule.config ? schedule.config.schedule : false}
}
...
async function currentPeriodProgress(period, id){
    ...
    let percentElapsed = (Date.now()/1000 - period["start_timestamp"]) / (period["end_timestamp"] - period["start_timestamp"])
...
async function currentPeriodText(period, id){
    if(period.href){
        document.getElementById(id).innerHTML = `<a href="${period.href}" target="_blank">${period["friendly_name"]}</a> ends at <b>${timestampToHuman(period["end_timestamp"])}</b> (in <b>${timestampsToLength(Date.now()/1000, period["end_timestamp"])}</b>).`
    }
    else{
        document.getElementById(id).innerHTML = `${period["friendly_name"]} ends at <b>${timestampToHuman(period["end_timestamp"])}</b> (in <b>${timestampsToLength(Date.now()/1000, period["end_timestamp"])}</b>).`}
    }
...
async function generateSchedule(periods, schedule){
    ...
    periods.forEach(period => {
        ...
        boxes.forEach(box => {
            if(!(prd.start_height >= box.height+box.start_height || prd.start_height+prd.height <= box.start_height)){
    ...
    return {boxes: boxes, height: height, columns: columns, start: start_timestamp, end: end_timestamp};
}
...
async function placeBoxes(data, schedule){
    ...
    while(time.getTime() < end.getTime()){
        ...
        times.push({time: time.getTime()/1000, offset: (time.getTime() - initial_time.getTime())/1000})
